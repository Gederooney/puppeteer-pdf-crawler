...
export const processBatch = async <
	T,
	U,
	V extends boolean | number | null | undefined = undefined
>(
	data: Array<T>,
	...
	const runOne = async (curIndex: number): Promise<void> => {
		...
		const statusResult = statusFunc
			? await statusFunc(curIndex, data.length, result)
			: true;
...
export function PromiseWaitAllNested<T>(
